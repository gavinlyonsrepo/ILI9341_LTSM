/*!
	@file   ILI9341_LTSM.hpp
	@author Gavin Lyons
	@brief  header file to manage hardware interface with ILI9341 display, ILI9341_LTSM arduino library
	@todo   Add support for touch screen controller xpt2046 IC.
*/
...
#define DISPLAY16_MISO_SetDigitalInput pinMode(_display_MISO, INPUT);

/*!@brief Class to interface with ILI9341 LCD display*/
class ILI9341_LTSM : public display16_graphics_LTSM
{
public:
	ILI9341_LTSM();
	~ILI9341_LTSM(){};
	...
	//Init Setup related
	void SetupGPIO_SPI(uint16_t CommDelay, int8_t RST, int8_t DC, int8_t CS, int8_t SCLK, int8_t MOSI, int8_t MISO = -1);
...
private:
	...
	bool _misoPinOn = false; /**< miso pin? true:Miso pin YES, false:Miso pin NO*/
	// Screen 
